...
for (i in 1:nvar){ #loop over each variable
  ...
  test_values <- matrix(rep(dimensionless_nominal_values, times = np), nrow = np, ncol = nvar, byrow = TRUE)
  ...
  #store the mean and upper and lower quartiles relative to observations
  normalized_yy_mean[,i] <- tmp$mean - normalized_observations
  normalized_yy_u95[,i] <- tmp$upper95 - normalized_observations
  normalized_yy_l95[,i] <- tmp$lower95 - normalized_observations
}
...
for (i in 1:nvar){
  ...
  p <- plot(0,0,xlim = c(min(dimensional_xx[,i]),max(dimensional_xx[,i])), ylim = c(-0.5,0.5),type = "n",xlab = input_colnames[i], ylab = "GrV error (/1e12)", cex = 1.1, cex.main = 1.5, cex.axis = 1.5, cex.lab = 1.5)
  abline(v = 0, lwd = 0.5)
  abline(h = 0, lwd = 0.5)
